...
class Graph:
    ...
    def get_neighbors(self, v):
        neighbors = []
        try:
            index = nameOfCity.index(v)
            for i in range(numberOfCities):
                if self.road_distances[index][i] > 0:
                    neighbor = nameOfCity[i]
                    distance = self.road_distances[index][i]
                    neighbors.append((neighbor, distance))
            return neighbors
        except ValueError:
            print("Could not find city with name in the list")

    # heuristic function with equal values for all nodes
    def h(self, n):
        if n in flight_distances_dict:
            return flight_distances_dict[n]
        else:
            return float('inf')
